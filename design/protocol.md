...
| Block | Fields |
|---|---|
| quiz | `questionIndex`, `questionCount`, `subState` (`answering` \| `revealed` \| `learningTextShown`), `question: {de, en}`, `answers: [{de, en}]`, `ownAnswerIndex?` — never `correctAnswerIndex` or `learningText`: the participant device shows only the participant's own answer after casting (Task 19a), and data a screen must not show is data not sent |
