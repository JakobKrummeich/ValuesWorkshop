...
**Presenter · Top-values bar chart (phase 4, fullscreen)**
...
│ and 4 more                                      │
│                                                 │
│ ordered by selections: ranks 1–10 left column   │
│ top→bottom, then ranks 11–20 right column       │
│ bar width ∝ selections; most-selected = full    │
│ ▓ = top value, color-highlighted                │
│ (tie at 10th → 11+ highlighted; a top set       │
│  wider than 20 grows the chart to show all)     │
│ "and x more" = selected values below the        │
│ 20-row cutoff, absent at 0; zero submissions    │
│ → heading + "Nobody submitted a selection."     │
└─────────────────────────────────────────────────┘
```
...
All three screens first show a progress bar (Task 19b). Its value is not
timed on the client: the server enters phase 5 unformed, emits the elapsed
fraction of a fixed 3-second window in the state (`design/protocol.md`
§ 5.1), and forms the groups when the window is over — regardless of actual
solve time. Advance is refused while the bar runs; the advance to group work
stays facilitator-triggered.
...
**Presenter · Group intro (fullscreen)**
...
Each group's block opens with this intro — on phase entry the walk stands
on the first group's intro; `GoToNextValue` steps intro → first value → …
→ last value → next group's intro. The intro between groups looks the
same as the intro on phase entry.
...
On a group intro the position line reads "Up next: Otter" — no actions,
