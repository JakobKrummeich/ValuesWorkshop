...
| **5 Group formation** | All groups: names, members, assigned values. **AdvancePhase** · System: FormGroups on entry | Fixed 3 s progress bar on entry (T19b), then paginated 3×2 group cards mirroring the participant card (name, member chips, value chips); cycles every 7 s, static single page when all groups fit | Fixed 3 s progress bar on entry (T19b), then own group card: animal name, member chips grouped top-left, value chips grouped bottom-right — one color for all member chips, another for all value chips, no section labels, no icon |
